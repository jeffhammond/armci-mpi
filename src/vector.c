                           ...
                           MPI_Datatype type, int proc, int blocking, armci_hdl_t * handle)
{
    ...
    switch(op) {
      case ARMCII_OP_PUT:
        gmr_put_typed(mreg, buf_loc[0], 1, type_loc, MPI_BOTTOM, 1, type_rem, proc, handle);
        flush_local = 1;
        break;
      case ARMCII_OP_GET:
        gmr_get_typed(mreg, MPI_BOTTOM, 1, type_rem, buf_loc[0], 1, type_loc, proc, handle);
        flush_local = 0;
        break;
      case ARMCII_OP_ACC:
        gmr_accumulate_typed(mreg, buf_loc[0], 1, type_loc, MPI_BOTTOM, 1, type_rem, proc, handle);
        flush_local = 1;
        break;
      default:
        ARMCII_Error("unknown operation (%d)", op);
